...
`
  ...
  expect(getNotes(recipe)).toHaveLength(1)
  expect(getNotes(recipe)[0]).toBe("This is a note about the recipe. Another note line.")
})
...
test("parse inline comments are stripped from output", () => {
  ...
  expect(getSteps(recipe)).toHaveLength(1)
  expect(
    getSteps(recipe)[0]?.every(i => i.type !== "text" || !i.value.includes("This is a comment")),
  ).toBe(true)
})
...
`
  ...
  expect(getNotes(recipe)).toHaveLength(1)
  expect(getNotes(recipe)[0]).toBe("First note. Second note. Third note.")
})
