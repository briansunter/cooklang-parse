...
function createSemantics() {
  ...
  semantics.addOperation('toAST', {
    Recipe(_metadata, sections, steps, notes, blockComments) {
      const metadataNode = _metadata.numChildren > 0
        ? _metadata.children[0].toAST() as Metadata
        : null;

      const sectionsList = sections.children.map((s: unknown) => (s as { toAST(): Section }).toAST());
    ...
    Step(lines) {
      const stepLines = lines.children.map((line: unknown) => (line as { toAST(): { text: string; ingredients: Ingredient[]; cookware: Cookware[]; timers: Timer[]; inlineComments: Comment[] } }).toAST());
    ...
    StepLine(items, inlineComment) {
      const stepItems = items.children.map((c: unknown) => (c as unknown as { toAST?: () => unknown }).toAST?.()).filter((c): c is NonNullable<typeof c> => c !== null && c !== undefined);
